...
document.addEventListener('DOMContentLoaded', function() {
  // Find all elements with the class 'authifyweb-widget-container'
  const containers = document.querySelectorAll('.authifyweb-widget-container');

  containers.forEach(function(container) {
    // Check if the widget already exists inside the container
    if (!container.querySelector('.authifyweb-widget')) {
      // Create the widget container and add it to the page
      const widgetContainer = document.createElement('div');
      widgetContainer.classList.add('authifyweb-widget'); // Add a class to identify it later
      ...
      widgetContainer.style.border = '2px solid #767676';
